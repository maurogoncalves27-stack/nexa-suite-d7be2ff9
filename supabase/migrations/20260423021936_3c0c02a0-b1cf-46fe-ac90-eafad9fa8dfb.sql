-- Drop the old prints-based importer
DROP FUNCTION IF EXISTS public.bulk_import_menu_items(jsonb);

-- New importer for Saipos XLSX (dishes + complements)
...
BEGIN
  ...
  FOR v_dish IN SELECT * FROM jsonb_array_elements(_dishes) LOOP
    ...
    -- Upsert PRODUCT for the dish (by name, case-insensitive)
    ...
    -- Build free complements list for recipe notes
    ...
    -- Upsert RECIPE for the dish (one per dish)
    ...
    -- Upsert POS MAPPING by dish name
    ...
    ON CONFLICT (pos_item_name) DO UPDATE
      SET recipe_id = EXCLUDED.recipe_id,
          ...
          updated_at = now()
    RETURNING (xmax = 0) INTO v_existing_product_id; -- reuse var: true=insert

    IF v_existing_product_id THEN
      v_mappings_created := v_mappings_created + 1;
    ELSE
      v_mappings_updated := v_mappings_updated + 1;
    END IF;

    -- Process PAID complements (price > 0): create as separate products
    FOR v_comp IN
      SELECT * FROM jsonb_array_elements(coalesce(v_dish->'complements', '[]'::jsonb)) c
      WHERE coalesce((c->>'price')::numeric, 0) > 0
    LOOP
...
GRANT EXECUTE ON FUNCTION public.import_saipos_menu(jsonb) TO authenticated;