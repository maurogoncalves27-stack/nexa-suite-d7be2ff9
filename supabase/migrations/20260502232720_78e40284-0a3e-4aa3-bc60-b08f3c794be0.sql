...
BEGIN
  ...
  outputs AS (
    SELECT
      ...
      jsonb_build_object(
        'auto_breakdown', COALESCE(a.store_breakdown, '[]'::jsonb),
        'manual_breakdown', COALESCE(m.detail, '[]'::jsonb)
      )                                         AS details
    FROM auto a
  ...
  SELECT
    ...
    (CASE WHEN jsonb_typeof(o.details->'auto_breakdown') = 'array'
          THEN jsonb_array_length(o.details->'auto_breakdown') ELSE 0 END
     + CASE WHEN jsonb_typeof(o.details->'manual_breakdown') = 'array'
          THEN jsonb_array_length(o.details->'manual_breakdown') ELSE 0 END) AS store_count,
    o.manual_qty,
    o.auto_qty,
    o.details
  FROM outputs o
