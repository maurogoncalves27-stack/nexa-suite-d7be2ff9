...
BEGIN
  ...
  WHILE (acq_start + INTERVAL '12 months')::DATE <= CURRENT_DATE LOOP
    ...
    -- guarda o último totalmente cumprido para fallback
  ...
  -- Se chegou aqui, todos os aquisitivos completados estão totalmente programados.
  -- Retorna o último completado para que a UI ainda mostre o estado, em vez de "1º ano".
  IF last_acq_start IS NOT NULL THEN
    until_dl := last_conc_end - CURRENT_DATE;
    IF until_dl < 0 THEN risk := 'expired';
    ELSIF until_dl <= 30 THEN risk := 'critical';
    ELSIF until_dl <= 60 THEN risk := 'warning';
    ELSE risk := 'ok';
