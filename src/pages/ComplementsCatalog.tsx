...
import { Layers, Plus, Trash2, Loader2, ChevronDown, ChevronUp, Search } from "lucide-react";
...
export default function ComplementsCatalog() {
  ...
  const [usage, setUsage] = useState<Record<string, number>>({}); // group_id -> N pratos
  const [openIds, setOpenIds] = useState<Set<string>>(new Set());
  const [search, setSearch] = useState("");

  const [editorOpen, setEditorOpen] = useState(false);
  const [editing, setEditing] = useState<Group | null>(null);
  const [form, setForm] = useState<{ name: string; min: number; max: number; required: boolean }>({
  ...
  async function toggleGroupActive(g: Group) {
    const { error } = await (supabase as any).from("complement_groups")
      .update({ is_active: !g.is_active }).eq("id", g.id);
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    toast({ title: g.is_active ? "Grupo pausado" : "Grupo ativado", description: "Reflete em todos os pratos." });
    load();
  }

  async function toggleOptionActive(o: Option) {
    const { error } = await (supabase as any).from("complement_options")
      .update({ is_active: !o.is_active }).eq("id", o.id);
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    load();
  }
  ...
  async function deleteGroup(g: Group) {
    ...
    if (n > 0) {
      if (!confirm(`Este grupo está vinculado a ${n} prato(s). Excluir vai REMOVER em todos eles. Confirmar?`)) return;
    } else if (!confirm("Excluir este grupo?")) return;
    const { error } = await (supabase as any).from("complement_groups").delete().eq("id", g.id);
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
  ...
  async function deleteOption(o: Option) {
    ...
    const { error } = await (supabase as any).from("complement_options").delete().eq("id", o.id);
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    load();
  }
  ...
  async function saveGroup() {
    ...
    if (editing) {
      const { error } = await (supabase as any).from("complement_groups").update(payload).eq("id", editing.id);
      if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    } else {
  ...
  async function addOption(groupId: string) {
    ...
    const { error } = await (supabase as any).from("complement_options").insert({
      group_id: groupId, name: name.trim(), extra_price: extra, sort_order: 0,
    });
    if (error) { toast({ title: "Erro", description: error.message, variant: "destructive" }); return; }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <Input placeholder="Buscar grupo..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8" />
        </div>
        <Button onClick={openNew} className="gap-2"><Plus className="h-4 w-4" /> Novo grupo</Button>
      </div>
                ...
                <CardContent className="p-3 space-y-2">
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant="outline" className="text-[10px]">{opts.length} opções</Badge>
                          <Badge variant="outline" className="text-[10px]">
                  ...
                  {isOpen && (
                    <div className="pl-10 space-y-1.5">
                      ...
                      {opts.map((o) => (
                        <div key={o.id} className={`flex items-center gap-2 text-sm py-1 ${!o.is_active ? "opacity-60" : ""}`}>
                          <Switch checked={o.is_active} onCheckedChange={() => toggleOptionActive(o)} />
                          <span className="flex-1 truncate">{o.name}</span>
                          {Number(o.extra_price) > 0 && (
                            <span className="text-xs tabular-nums text-muted-foreground">+{fmt(Number(o.extra_price))}</span>
                          )}
                          {!o.is_active && <Badge variant="outline" className="text-[10px]">Pausado</Badge>}
                          <Button size="icon" variant="ghost" onClick={() => deleteOption(o)}>
                            <Trash2 className="h-4 w-4 text-destructive" />
                          </Button>
                        </div>
                      ))}
                      <Button size="sm" variant="outline" onClick={() => addOption(g.id)} className="gap-1 mt-1">
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              <Label>Nome</Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Ex: Acompanhamentos" autoFocus />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <div className="space-y-1.5">
                <Label>Mínimo</Label>
                <Input type="number" min="0" value={form.min} onChange={(e) => setForm({ ...form, min: Number(e.target.value) || 0 })} />
              </div>
              <div className="space-y-1.5">
                <Label>Máximo</Label>
                <Input type="number" min="1" value={form.max} onChange={(e) => setForm({ ...form, max: Number(e.target.value) || 1 })} />
              </div>
            </div>
