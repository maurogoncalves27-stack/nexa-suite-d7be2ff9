...
export default function QuickInfractionButton({ trigger }: Props = {}) {
  ...
  const save = async () => {
    ...
    // Notifica o colaborador via push (cria um aviso direcionado)
    try {
      ...
      <DialogTrigger asChild>
        {trigger ?? (
          <Button variant="destructive" className="w-full sm:w-auto gap-2">
            <ShieldAlert className="h-4 w-4" /> Registrar infração
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter className="flex-row items-center justify-between gap-2 sm:justify-between">
          <div className="flex items-center gap-2">
            <Label htmlFor="infraction-date" className="text-xs whitespace-nowrap">Data</Label>
            <Input
              id="infraction-date"
              type="date"
              value={occurredOn}
