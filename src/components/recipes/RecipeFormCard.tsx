...
const RecipeFormCard = ({ recipeId, defaultOpen, initialBrandId, hideFactory, factoryMode, onSaved, onCancelNew, onDeleted, onDuplicated }: Props) => {
  ...
  const syncBrands = async (rid: string) => {
    if (factoryMode) {
      // Fichas da CD não têm vínculo com marcas — geram porções transferidas às lojas
              ...
              <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="prep">Pré-preparo (insumo usado por outra ficha da CD)</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    {factoryRecipeKind === "porcao" ? (
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-muted-foreground">
                          A CD não tem cardápio — o item porcionado é o "cardápio da CD". Só produtos da categoria <strong>PORCIONADOS</strong> aparecem aqui. As porções produzidas são transferidas às lojas via Solicitações da CD.
                        </p>
                      </div>
                    ) : (
                      <div className="rounded-md border border-dashed bg-muted/40 p-3 text-[11px] text-muted-foreground">
                        Pré-preparo da CD: não gera item porcionado. Esta ficha será usada como ingrediente dentro de outra ficha da CD.
                      </div>
                    )}
                  </>
                ...
                {factoryMode ? (
                  <div className="rounded-md border border-dashed bg-muted/40 p-3 text-sm text-muted-foreground">
                    Esta ficha gera <strong>porções</strong> que são transferidas às lojas via Solicitações da CD.
                    A CD não tem cardápio — defina o <em>produto de saída</em> acima para que as porções entrem no estoque das lojas que solicitarem.
                  </div>
                ) : (
                  <>
